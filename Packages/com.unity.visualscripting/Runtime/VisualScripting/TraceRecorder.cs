...
{
    public class TraceRecorder : IEnumerator
    {
        readonly ushort m_MaxFrames;
        CircularBuffer<KeyValuePair<int, object>> m_Frames;
        static KeyComparer<int, object> s_KeyComparer = new KeyComparer<int, object>();

        class KeyComparer<T, TU> : IComparer<KeyValuePair<T, TU>> {
            public int Compare(KeyValuePair<T, TU> x, KeyValuePair<T, TU> y)
            {
                return Comparer<T>.Default.Compare(x.Key, y.Key);
            }
        }

        public TraceRecorder(ushort maxFrames)
        {
        ...
        public object Current => m_Frames.Last().Value;

        public object Get(int timestamp)
        {
            if (m_Frames == null)
                return null;

            if(m_Frames.BinarySearch(new KeyValuePair<int, object>(timestamp, null), s_KeyComparer, out var data))
                return data.Value;
            return null;

        }

        public void Record(int timestamp, object obj)
        {
            if (m_Frames == null)
                m_Frames = new CircularBuffer<KeyValuePair<int, object>>(m_MaxFrames);

            m_Frames.PushBack(new KeyValuePair<int, object>(timestamp, obj));
        }
    }
