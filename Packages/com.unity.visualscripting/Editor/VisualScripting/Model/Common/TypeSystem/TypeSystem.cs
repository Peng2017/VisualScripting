...
namespace UnityEditor.VisualScripting.Model
{
    public static class TypeSystem
    {
        ...
        public static Dictionary<Type, List<MethodInfo>> GetExtensionMethods(IEnumerable<Assembly> assemblies)
        {
            ...
            var methodInfos = TypeCache.GetMethodsWithAttribute<ExtensionAttribute>()
                .Where(m => assemblies.Contains(m.DeclaringType?.Assembly))
                .OrderBy(m => m.Name)
                .ToList();
            ...
            foreach (var methodInfo in methodInfos)
            {
                ...
                Type type = parameterType.IsArray ? parameterType.GetElementType() : parameterType;
                ...
                extensionMethods[type].Add(methodInfo);
            }
    ...
    {
        public static Type GetVsArrayElementType(this Type type)
        {
            return type.GetGenericArguments().Single();
        }

        public static Type MakeVsArrayType(this Type type)
        {
            return typeof(VSArray<>).MakeGenericType(type);
        }

        public static bool IsVsArrayType(this Type type)
        {
            return type.IsConstructedGenericType && type.GetGenericTypeDefinition() == typeof(VSArray<>);
        }

        public static bool IsVsArrayCompatible(this Type type)
        {
            return typeof(IVSArray).IsAssignableFrom(type) || type.IsArray || typeof(ICollection).IsAssignableFrom(type);
        }
    }
}
