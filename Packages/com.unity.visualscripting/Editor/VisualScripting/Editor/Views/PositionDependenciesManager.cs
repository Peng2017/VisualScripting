...
namespace UnityEditor.VisualScripting.Editor
{
    ...
    class PositionDependenciesManager
    {
        ...
        readonly Dictionary<INodeModel, Dictionary<INodeModel, IDependency>> m_DependenciesByNode = new Dictionary<INodeModel, Dictionary<INodeModel, IDependency>>();
        ...
        {
            if (!m_DependenciesByNode.TryGetValue(parent, out Dictionary<INodeModel, IDependency> link))
                m_DependenciesByNode.Add(parent, new Dictionary<INodeModel, IDependency> { {child.DependentNode, child }});
            else
            {
                if (link.TryGetValue(child.DependentNode, out IDependency dependency))
                {
                ...
                {
                    link.Add(child.DependentNode, child);
                }
            }
        }
        ...
        {
            if (!m_DependenciesByNode.TryGetValue(parent, out var link))
                return null;
        ...
        public void Remove(INodeModel parent, INodeModel child)
        {
            if (!m_DependenciesByNode.TryGetValue(parent, out Dictionary<INodeModel, IDependency> link))
                return;
            if (!link.TryGetValue(child, out IDependency dependency))
                return;
            if (dependency is LinkedNodesDependency linked)
            {
                linked.count--;
        ...
        public void Clear()
        {
            foreach (KeyValuePair<INodeModel, Dictionary<INodeModel, IDependency>> pair in m_DependenciesByNode)
                pair.Value.Clear();
        ...
        {
            Log("Dependencies :" + String.Join(", ", m_DependenciesByNode.Where(n => (n.Key as NodeModel) != null).Select(n => $"{n.Key.Title}: {n.Value.ToString()}")));
        }
        ...
        void ProcessDependency(INodeModel nodeModel, Vector2 delta, Action<GraphElement, IDependency, Vector2, INodeModel> dependencyCallback)
        {
            ...
            if (!m_DependenciesByNode.TryGetValue(nodeModel, out Dictionary<INodeModel, IDependency> link))
                return;

            foreach (KeyValuePair<INodeModel, IDependency> dependency in link)
            {
        ...
        public void StopNotifyMove()
        {
            ProcessMovedNodes(Vector2.zero, (element, model, _, __) =>
            {
                ...
                Undo.RecordObject((Object)model.DependentNode, "Move dependency");
            ...
            switch (dependency)
            {
                case LinkedNodesDependency linked:
                        ...
                        element is Experimental.GraphView.StackNode)
                    {
                        ...
                        {
                            int n = prev.OutputPortModels.Count;

                            // distribute stacks horizontally around the parent's output port
                            float nn = 1.0f / (n + 1);

                            float offset = (linked.ParentPort.Index + 1) * nn * 2.0f - 1.0f;
                    ...
                    Undo.RecordObject((Object)linked.DependentNode, "Align node");
        ...
        public void AlignNodes(VseGraphView vseGraphView, bool follow, List<ISelectable> selection)
        {
            ...
            if (!anyEdge && !follow)
            {
                ...
                {
                    if (!m_DependenciesByNode.TryGetValue(model, out Dictionary<INodeModel, IDependency> dependencies))
                        continue;
                    foreach (KeyValuePair<INodeModel, IDependency> dependency in dependencies)
                    {
