...
namespace UnityEditor.VisualScripting.Editor
{
    public sealed class VseUIController
    {
        ...
        internal void RemoveFromGraphView(GraphElement graphElement)
        {
            switch (graphElement) {
                ...
                case Node n when n.Stack != null:
                    m_GraphView.RemovePositionDependencies(n.Stack.stackModel, new[]{n.model});
                    break;
            }
        ...
        void RebuildAll(State state)
        {
            ...
            var allPorts = m_GraphView.ports.ToList().ToDictionary(p => p.userData as PortModel);
            m_GraphView.PositionDependenciesManagers.portModelToPort = allPorts;
            int index = 0;
        ...
        public void DisplayCompilationErrors(State state)
        {
            ...
            {
                INodeModel model = error.sourceNode as INodeModel;

                if (model == null)
                    continue;

                var alignment = model is StackModel
...
//            blackboard.ClearContents();
        ...
        void AddToGraphView(GraphElement graphElement, Experimental.GraphView.Scope scope = null)
        {
            ...
            if (graphElement is StackNode || graphElement is Node || graphElement is Token || graphElement is Group
                || graphElement is Edge)
            ...
            foreach (ISelectable x in selection)
            {
                switch (x)
                {
                    ...
                    case StackNode stackNode:
                    {
                        ...
                        {
                            FetchVariableDeclarationModelsFromFunctionModel(functionNode.functionModel, ref declarationModelTuples);
                        }
                        else
                        {
                            List<ISelectable> connectedInputNodes = new List<ISelectable>();
                            foreach (var inputPortModel in stackNode.stackModel.InputPortModels)
                    ...
                    case Node node:
                        if (node.Stack != null && visited.Add(node.Stack.stackModel))
                        {
                            if (node.Stack is FunctionNode parentFunctionNode)
                                FetchVariableDeclarationModelsFromFunctionModel(parentFunctionNode.functionModel, ref declarationModelTuples);
                            else
        ...
        // TODO: UnityEditor.Experimental.GraphView.Blackboard already defines these default values but
        //       they are unavailable since defined as private.
        public const int k_DefaultBlackboardWidth = 300;
        public const float k_DefaultBlackboardHeight = 400;

        public void ResetBlackboard()
        {
            Blackboard.SetPosition(new Rect(0f, 0f, k_DefaultBlackboardWidth, k_DefaultBlackboardHeight));
        }
    }
}
