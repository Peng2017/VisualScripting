...
namespace UnityEditor.VisualScripting.Editor
{
    public class VseGraphView : GraphView, IDropTarget
    {
        ...
        public VseGraphView(VseWindow window, Store store)
        {
            ...
            Add(UIController.Blackboard);

            elementResized = OnElementResized;

            contentViewContainer.RegisterCallback<GeometryChangedEvent>(OnGeometryChanged);
        ...
        {
            if (!(element is GraphElement ce) || ce != UIController.Blackboard)
                return;
            ...
            if (ce == UIController.Blackboard)
            {
                IEditorDataModel editorDataModel = m_Store.GetState().EditorDataModel;
                editorDataModel.BlackboardPosition = ce.GetPosition();
            }
        }
        ...
        GraphViewChange OnGraphViewChanged(GraphViewChange graphViewChange)
        {
            ...
            var groupModelsToMove = new HashSet<GroupNodeModel>();
            ...
            foreach (var movedElement in movableElements)
            {
                ...
                {
                    case GroupNodeModel groupModel:
                        groupModelsToMove.Add(groupModel);

                        // We need to also move the nodes in the group (since they're not actually parented).
                        nodeModelsToMove.AddRange(groupModel.NodeModels.Cast<NodeModel>());
                        break;
            ...
            if (nodeModelsToMove.Any() || groupModelsToMove.Any() || stickyModelsToMove.Any())
                m_Store.Dispatch(new MoveElementsAction(delta, nodeModelsToMove, groupModelsToMove, stickyModelsToMove));
        ...
        {
            UIController.Blackboard.style.maxWidth = layout.width;
            UIController.Blackboard.style.maxHeight = layout.height;
        }
        ...
        [Serializable]
        internal struct StackedNodesStruct
        {
            public ScriptableObject parentStackNodeModel;
            public ScriptableObject stackedNodeModel;
        }

        [Serializable]
        internal class CopyPasteData
        {
            public List<ScriptableObject> nodes;
            public List<StackedNodesStruct> implicitStackedNodes;
            public List<EdgeModel> edges;
            public List<ScriptableObject> variableDeclarations;
            public Vector2 topLeftNodePosition;
            public List<ScriptableObject> stickyNotes;
        ...
        protected override void CollectCopyableGraphElements(IEnumerable<GraphElement> elements, HashSet<GraphElement> elementsToCopySet)
        {
            ...
            List<GraphElement> elementsToRemove =
                elementsToCopySet.Where(e => e is FunctionNode node && !node.functionModel.AllowMultipleInstances).ToList();
        ...
        {
            var copyPasteData = GatherCopiedElementsData(o =>
            {
                var clone = Object.Instantiate(o);
                clone.name = clone.name.Substring(0, clone.name.Length - k_CloneStringLength);

                return clone;
            }, elements
                .OfType<IHasGraphElementModel>().Select(x => x.GraphElementModel)
                .ToList());
            return copyPasteData.IsEmpty() ? string.Empty : copyPasteData.ToJson();
        }
        ...
        internal static void Duplicate(VSGraphModel graphModel, IReadOnlyCollection<IGraphElementModel> elementModels, out Dictionary<INodeModel, NodeModel> mapping)
        {
            ...
            CopyPasteData data = GatherCopiedElementsData(x => x, elementModels);
            if (data.IsEmpty())
            {
        ...
        internal static CopyPasteData GatherCopiedElementsData(Func<ScriptableObject, ScriptableObject> map, IReadOnlyCollection<IGraphElementModel> graphElementModels)
        {
            IEnumerable<INodeModel> originalNodes = graphElementModels
                .OfType<INodeModel>()
            ...
            List<ScriptableObject> nodesToCopy = originalNodes
                .OfType<ScriptableObject>()
                .Select(map)
                .ToList();
            List<StackedNodesStruct> implicitStackedNodesToCopy = new List<StackedNodesStruct>();
            int nodeIndex = 0;
            foreach (ScriptableObject node in nodesToCopy)
            {
                ((INodeModel)node).OriginalInstanceID = ((Object)originalNodes.ElementAt(nodeIndex++)).GetInstanceID();
                if (node is IStackModel stackModel)
                {
                    foreach (INodeModel stackedNodeModel in stackModel.NodeModels)
                    {
                        ScriptableObject stackedNodeModelToCopy = map((ScriptableObject)stackedNodeModel);

                        // Reset name to be the exact same as the original since they are in different scopes
                        stackedNodeModelToCopy.name = ((NodeModel)stackedNodeModel).name;
                        ((INodeModel)stackedNodeModelToCopy).OriginalInstanceID = ((Object)stackedNodeModel).GetInstanceID();

                        implicitStackedNodesToCopy.Add(new StackedNodesStruct
                        {
                            parentStackNodeModel = (ScriptableObject)stackModel,
                            stackedNodeModel = stackedNodeModelToCopy
                        });
                    }
                }
            }
            ...
            List<ScriptableObject> variableDeclarationsToCopy = graphElementModels
                .OfType<IVariableDeclarationModel>()
                .OfType<ScriptableObject>()
                .Select(map)
                .ToList();

            List<ScriptableObject> stickyNotesToCopy = graphElementModels
            ...
            CopyPasteData copyPasteData = new CopyPasteData
            {
                topLeftNodePosition = floatingNodes.Skip(1).Aggregate((floatingNodes.FirstOrDefault()?.Position) ??
                    (stickyNotesToCopy.FirstOrDefault() as
        ...
        public static void OnUnserializeAndPaste(VSGraphModel graph, TargetInsertionInfo targetInfo, IEditorDataModel editorDataModel, string data)
        {
            OnUnserializeAndPaste(graph, targetInfo, editorDataModel, data, out _);
        }

        static void OnUnserializeAndPaste(VSGraphModel graph, TargetInsertionInfo targetInfo, IEditorDataModel editorDataModel, string data, out Dictionary<INodeModel, NodeModel> mapping)
        {
        ...
        {
            mapping = new Dictionary<INodeModel, NodeModel>();
            foreach (var original in copyPasteData.nodes.Cast<INodeModel>())
            {
                if (!(Object)original)
                    continue;

                // pasted, stacked node
                if ((!original.OutputPortModels.Any() || original.IsBranchType) && !isDuplication)
                {
                    ...
                    PasteNode(targetInfo.OperationName, original, mapping, targetInfo.TargetStack, graph,
                ...
                {
                    PasteNode(targetInfo.OperationName, original, mapping, original.ParentStackModel as StackModel,
                        graph, editorDataModel, targetInfo.Delta,
                        targetInfo.TargetStackInsertionIndex != -1 ? targetInfo.TargetStackInsertionIndex++ : -1,
                        copyPasteData.implicitStackedNodes
                            .Where(x => x.parentStackNodeModel == (ScriptableObject)original).ToList());
                }
            }
            ...
            foreach (var edge in copyPasteData.edges)
            {
                ...
                {
                    var inputPortModel = newInput.InputPortModels.ElementAt(edge.InputPortModel.Index);
                    var outputPortModel = newOutput.OutputPortModels.ElementAt(edge.OutputPortModel.Index);
            ...
            foreach (var stickyNote in copyPasteData.stickyNotes.Cast<IStickyNoteModel>())
            {
                ...
                var pastedStickyNote = (StickyNoteModel)graph.CreateStickyNote(stickyNote.Title, newPosition);
            ...
            Undo.RegisterCompleteObjectUndo(graph, $"{targetInfo.OperationName} selection");
        }
        ...
        void UnserializeAndPaste(string operationName, string data)
        {
            ...
            IEditorDataModel editorDataModel = m_Store.GetState().EditorDataModel;
            m_Store.Dispatch(new PasteSerializedDataAction(graph, info, editorDataModel, data));
        }

        static void PasteNode(string operationName, INodeModel copiedNode, Dictionary<INodeModel, NodeModel> mapping,
            StackModel pastedNodeParentStack, VSGraphModel graph, IEditorDataModel editorDataModel, Vector2 delta,
            int stackInsertionIndex,
            List<StackedNodesStruct> implicitStackedNodes = null)
        {
            var pastedNodeModel = (NodeModel)Object.Instantiate((Object)copiedNode);
            Undo.RegisterCreatedObjectUndo(pastedNodeModel, "Duplicate node");
            pastedNodeModel.name = ((Object)copiedNode).name;
            ...
            if (copiedNode.ParentStackModel == null)
            {
                graph.AddNode(pastedNodeModel);
                pastedNodeModel.Position += delta;

                StackModel pastedStackModel = pastedNodeModel as StackModel;
                if (pastedStackModel != null)
                {
                    ...
                    if (implicitStackedNodes?.Count > 0)
                    {
                        foreach (var stackedNodeStruct in implicitStackedNodes)
                            PasteNode(operationName, (INodeModel)stackedNodeStruct.stackedNodeModel, mapping,
                                pastedStackModel, graph, editorDataModel, delta, stackInsertionIndex != -1 ? stackInsertionIndex++ : -1);
                    }

                    var copiedFunctionModel = copiedNode as FunctionModel;
                    var pastedFunctionModel = pastedStackModel as FunctionModel;
                    if (copiedFunctionModel != null && pastedFunctionModel != null)
                    {
                        ...
                        {
                            VariableDeclarationModel variableDeclaration = pastedFunctionModel.CreateFunctionVariableDeclaration(functionVariableModel.Name,
                                functionVariableModel.DataType);
                        ...
                        {
                            VariableDeclarationModel parameterDeclaration = (VariableDeclarationModel)GameObject.Instantiate((Object)functionParameterModel);
            ...
            Undo.RegisterCreatedObjectUndo(pastedNodeModel, operationName);

            if (pastedNodeParentStack != null)
                pastedNodeParentStack.AddStackedNode(pastedNodeModel, stackInsertionIndex);

            if ((Object)graph.AssetModel)
                Utility.SaveAssetIntoObject(pastedNodeModel, (Object)graph.AssetModel);
        }
        ...
        public bool DragPerform(DragPerformEvent evt, IEnumerable<ISelectable> dragSelection, IDropTarget dropTarget, ISelection dragSource)
        {
            ...
            // Notify all elements that used to belong to a group that they no longer do
            INodeModel[] recentlyUngroupedElementModels =
                dropElements
                    .Where(e => e.GetContainingScope() == null)
                    .OfType<IHasGraphElementModel>()
                    .Select(e => e.GraphElementModel)
                    .OfType<INodeModel>()
                    .Where(m => !(m is IGroupNodeModel) && m.IsGrouped)
                    .ToArray();

            if (recentlyUngroupedElementModels.Any())
                m_Store.Dispatch(new RemoveFromGroupNodeAction(recentlyUngroupedElementModels));
            ...
            foreach (Node node in droppedNodes)
            {
                if (node.IsInStack)
                {
                    ...
                    if (node.selectedIndex != newIndex)
                        m_Store.Dispatch(new MoveStackedNodesAction(new []{node.model}, (StackModel)node.Stack.stackModel, newIndex));
                }
            }
...
//            return newTokenDeclaration;
                    ...
                    foreach (var dropElement in dragSelection.OfType<VisualElement>())
                    {
                        ...
                        if (dragSource is Blackboard)
                        {
                            ...
                            if (dropElement is IVisualScriptingField blackboardField)
                                actualDropElement = DetachConvertedBlackboardField(blackboardField);
                        }
                        else
                        {
            ...
            return ports.ToList().Where(p =>
                {
                    ...
                    if (outputNode.IsInsertLoop && inputNode is StackModel && inputPort.PortType == PortType.Execution)
                    {
                        switch (inputNode)
                        {
        ...
        public EventPropagation RemoveSelection()
        {
            ...
            INodeModel[] connectedNodes = selectedNodes.Where(x => x.InputPortModels
                    .Any(y => y.Connected) && x.OutputPortModels.Any(y => y.Connected))
                .ToArray();
        ...
        public void OnMouseOver(MouseOverEvent evt)
        {
            ...
            while (lastHoveredVisualElement != null &&
                !(lastHoveredVisualElement is Node ||
                    ...
                    lastHoveredVisualElement is Group ||
            ...
            while (lastHoveredSmartSearchCompatibleElement != null &&
                !(lastHoveredSmartSearchCompatibleElement is Node ||
                    ...
                    lastHoveredSmartSearchCompatibleElement is Group ||
        ...
        public void NotifyTopologyChange(IGraphModel graphModel)
        {
            ...
            UIController.Blackboard?.NotifyTopologyChange(graphModel);
        }
        ...
        void OnElementsInsertedToStackNode(Experimental.GraphView.StackNode graphStackNode, int index, IEnumerable<GraphElement> elements)
        {
            ...
            m_Store.Dispatch(new MoveStackedNodesAction(nodeElements.Select(n => n.model).ToList(), (StackModel)dropStack.stackModel, index));
        }

        void OnElementsRemovedFromStackNode(Experimental.GraphView.StackNode graphStackNode, IEnumerable<GraphElement> elements)
        {
            RemovePositionDependencies(((StackNode)graphStackNode).stackModel, elements.Cast<IHasGraphElementModel>().Select(x => x.GraphElementModel).Cast<INodeModel>());
        }
        ...
        public override void AddToSelection(ISelectable selectable)
        {
            ...
            if (m_PersistedSelectionRestoreEnabled &&
                ...
                hasModel.GraphElementModel is Object obj)
        ...
        public void RemovePositionDependencies(IStackModel stackModel, IEnumerable<INodeModel> removedNodes)
        {
            foreach (var stackedNode in removedNodes)
                PositionDependenciesManagers.Remove(stackModel, stackedNode);
            PositionDependenciesManagers.LogDependencies();
        }

        public void RemovePositionDependency(IEdgeModel model)
        {
            if (model.InputPortModel.NodeModel is IStackModel && (model.OutputPortModel.NodeModel is IStackModel ||  model.OutputPortModel.NodeModel.IsBranchType))
                PositionDependenciesManagers.Remove(model.OutputPortModel.NodeModel, model.InputPortModel.NodeModel);
            else
                PositionDependenciesManagers.Remove(model.InputPortModel.NodeModel, model.OutputPortModel.NodeModel);
            PositionDependenciesManagers.LogDependencies();
        }
        ...
        public void PanToNode(int nodeInstanceId)
        {
            // Find the model associated with the semantic node's instance id
            Object model = EditorUtility.InstanceIDToObject(nodeInstanceId);

            GraphElement graphElement = this.Query<Node>().Where(e => ReferenceEquals(e.model, model)).First();
            if (graphElement == null)
            {
                var methodUINodes = this.Query<FunctionNode>().ToList();
                graphElement = methodUINodes?.FirstOrDefault(e => e.functionModel as Object == model);
            }

            if (graphElement == null)
            {
                var stackUINodes = this.Query<StackNode>().ToList();
                graphElement = stackUINodes?.FirstOrDefault(e => ReferenceEquals(e.GraphElementModel, model));
            }

            if (graphElement == null)
                graphElement = this.Query<Token>().Where(e => ReferenceEquals(e.GraphElementModel, model)).First();
