...
namespace UnityEditor.VisualScripting.Editor
{
    class VseContextualMenuBuilder
    {
        ...
        public void BuildContextualMenu()
        {
            ...
            if (!originatesFromBlackboard || m_Evt.target is IHasGraphElementModel)
            {
                ...
                if (!originatesFromBlackboard)
                {
                    ...
                    BuildMakeArrayNodeContextualMenu(selectedModelsKeys);
                }
        ...
        {
            if (!(m_Evt.target is GraphView) && !(m_Evt.target is Group))
                return;
        ...
        void BuildNodeContextualMenu(Dictionary<IGraphElementModel, IHasGraphElementModel> selectedModels)
        {
            ...
            var connectedModels = models.Where(x => x.InputPortModels.Any(y => y.Connected) && x.OutputPortModels.Any(y => y.Connected)).ToArray();
            ...
            m_Evt.menu.AppendAction("Ungroup", menuAction =>
                {
                    m_Store.Dispatch(new RemoveFromGroupNodeAction(models));
                },
                evt => models.Any(m => !(m is GroupNodeModel) && m.IsGrouped) ? DropdownMenuAction.Status.Normal : DropdownMenuAction.Status.Disabled);
...
//                }, x => DropdownMenuAction.Status.Normal);
            ...
            var ungroupedModels = models.Where(x => !(x is GroupNodeModel) && !x.IsGrouped && !x.IsStacked).ToArray();

            if (ungroupedModels.Any() || canSelectionBeBypassed)
                m_Evt.menu.AppendSeparator();

            if (ungroupedModels.Any())
            {
                m_Evt.menu.AppendAction("Group", menuAction => m_Store.Dispatch(new CreateGroupNodeAction("Group", menuAction.eventInfo.mousePosition, ungroupedModels)),
                    eventBase => DropdownMenuAction.Status.Normal);
                m_Evt.menu.AppendSeparator();
            }
        ...
        void BuildEdgeContextualMenu(IReadOnlyCollection<IGraphElementModel> selectedModels)
        {
            IEdgeModel firstMatchingModel = selectedModels
                                                .OfType<IEdgeModel>()
                                                .FirstOrDefault(x => x.InputPortModel?.NodeModel is StackModel &&
                                                                     x.OutputPortModel?.NodeModel is StackModel);
            if (firstMatchingModel != null)
            {
                m_Evt.menu.AppendAction("Edge/Merge", menuAction => m_Store.Dispatch(new MergeStackAction(
                            (StackModel)firstMatchingModel.OutputPortModel.NodeModel,
                            (StackModel)firstMatchingModel.InputPortModel.NodeModel)),
                    eventBase => DropdownMenuAction.Status.Normal);
            }
        }

        void BuildMakeArrayNodeContextualMenu(IReadOnlyCollection<IGraphElementModel> selectedModels)
        {
            var models = selectedModels.Where(x => x is MakeArrayNodeModel).Cast<MakeArrayNodeModel>().ToArray();
            if (!models.Any())
                return;

            m_Evt.menu.AppendAction("Make Array/Add 1 Port", menuAction => m_Store.Dispatch(new ChangeMakeArrayNodePortCountAction(1, models)), eventBase => DropdownMenuAction.Status.Normal);
            m_Evt.menu.AppendAction("Make Array/Add 5 Ports", menuAction => m_Store.Dispatch(new ChangeMakeArrayNodePortCountAction(5, models)), eventBase => DropdownMenuAction.Status.Normal);
            m_Evt.menu.AppendAction("Make Array/Remove 1 Port", menuAction => m_Store.Dispatch(new ChangeMakeArrayNodePortCountAction(-1, models)), eventBase => DropdownMenuAction.Status.Normal);
            m_Evt.menu.AppendAction("Make Array/Remove 5 Ports", menuAction => m_Store.Dispatch(new ChangeMakeArrayNodePortCountAction(-5, models)), eventBase => DropdownMenuAction.Status.Normal);
            m_Evt.menu.AppendAction("Make Array/Reset", menuAction => m_Store.Dispatch(new ChangeMakeArrayNodePortCountAction(-100000, models)), eventBase => DropdownMenuAction.Status.Normal);
        }
        ...
        void BuildStickyNoteContextualMenu()
        {
            ...
            List<IStickyNoteModel> stickyNoteModels = stickyNoteSelection.Select(m => m.stickyNoteModel).ToList();
