...
namespace UnityEditor.VisualScripting.Editor
{
    static class GraphReducers
    {
        ...
        static State DeleteElements(State previousState, DeleteElementsAction action)
        {
            ...
            Undo.RegisterCompleteObjectUndo(graphModel, "Delete elements");
        ...
        static void DeleteElementsFromGraph(State previousState, IReadOnlyCollection<IGraphElementModel> elementsToRemove, GraphModel graphModel)
        {
            ...
            AnalyticsHelper.Instance.FlushNodeCreationEvent(NodeCreationEvent.UndoOrDelete, nodesToDelete.Select(n => ((Object)n).GetInstanceID()));
        ...
        static State MoveElements(State previousState, MoveElementsAction action)
        {
            ...
            if (action.GroupModels != null)
                foreach (var groupNodeModel in action.GroupModels)
                    groupNodeModel.MoveDelta(action.Delta);
        ...
        {
            previousState.EditorDataModel.NodeToFrameInstanceId = action.NodeInstanceId;
            previousState.MarkForUpdate(UpdateFlags.GraphGeometry);
            return previousState;
        }
    }
