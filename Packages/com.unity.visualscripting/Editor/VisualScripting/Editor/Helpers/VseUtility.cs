...
namespace UnityEditor.VisualScripting.Editor
{
    public static class VseUtility
    {
        ...
        internal static void UpdateCodeViewer(bool show, CompilationResult compilationResult, Action<object> selectionDelegate, SourceCodePhases sourceIndex = SourceCodePhases.Initial, Type pluginIndex = null)
        {
            ...
            IList<string> splitSourceCode;

            if (pluginIndex != null)
                splitSourceCode = compilationResult.pluginSourceCode[pluginIndex].Split(new[] {Environment.NewLine}, StringSplitOptions.None);
            else
                splitSourceCode = compilationResult.sourceCode[(int)sourceIndex].Split(new[] {Environment.NewLine}, StringSplitOptions.None);

            Dictionary<int, List<CompilerError>> errorPerLine = new Dictionary<int, List<CompilerError>>();
            ...
            for (var lineIndex = 0; lineIndex < splitSourceCode.Count; lineIndex++)
            {
                ...
                if (errorPerLine.ContainsKey(lineIndex))
                {
                    ...
                    var sourceNode = errorPerLine[lineIndex].FirstOrDefault()?.sourceNode as ScriptableObject;
                    if (sourceNode)
                        line.Metadata = sourceNode.GetInstanceID();
                }
