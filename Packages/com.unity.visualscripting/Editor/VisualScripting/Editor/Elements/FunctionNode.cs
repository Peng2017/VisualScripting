...
namespace UnityEditor.VisualScripting.Editor
{
    public class FunctionNode : StackNode, IRenamable
    {
        ...
        protected internal readonly IFunctionModel functionModel;
        ...
        public override bool IsRenamable() => base.IsRenamable() && functionModel.AllowChangesToModel;
        ...
        public string TitleValue => functionModel.Title;
            ...
            : base(store, functionModel, nodeBuilder)
        {
            ...
            else
            {
                ...
                RegisterCallback<AttachToPanelEvent>(evt => m_TitleLabel.Bind(new SerializedObject((Object) functionModel)));
                RegisterCallback<DetachFromPanelEvent>(evt => m_TitleLabel.Unbind());
            }
        ...
        void BuildVariables()
        {
            foreach (IPortModel inputPortModel in functionModel.InputPortModels)
            {
            ...
            foreach (var outputPortModel in functionModel.OutputPortModels)
            {
            ...
            foreach (var local in functionModel.FunctionParameterModels)
            {
                if(!(local as UnityEngine.Object) || local.IsExposed)
                    continue;
            ...
            foreach (var local in functionModel.FunctionVariableModels)
            {
                var tokenDeclaration = new TokenDeclaration(m_Store, local, m_GraphView);
                if (local is LoopVariableDeclarationModel loopVariableDeclarationModel)
                    VseUtility.AddTokenIcon(tokenDeclaration, loopVariableDeclarationModel.TitleComponentIcon);
        ...
        {
            bool canAddParam = functionModel.AllowChangesToModel;
        ...
        internal void CreateFunctionField(TypeHandle type, SupportedFields field)
        {
            if(field == SupportedFields.Variable)
                m_Store.Dispatch(new CreateFunctionVariableDeclarationAction((IFunctionModel)stackModel, field.ToString(), type));
            else
                m_Store.Dispatch(new CreateFunctionParameterDeclarationAction((IFunctionModel)stackModel, field.ToString(), type));
        }
