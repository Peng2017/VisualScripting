...
//        PersistedProperties m_PersistedProperties;
        ...
        public VseGraphView GraphView { get; }

        IBlackboardProvider m_LastProvider;

        const string k_ClassLibraryTitle = "Blackboard";

        const string k_PersistenceKey = "Blackboard";

        Label m_VseTitleLabel;

        public Blackboard(Store store, VseGraphView graphView)
        {
            Store = store;
            GraphView = graphView;
            ...
            m_VseTitleLabel = this.Q<Label>("titleLabel");
        }

        void StoreOnStateChanged()
        {
            IGraphModel graphModel = Store.GetState()?.CurrentGraphModel;
            if (m_VseTitleLabel.IsBound())
                m_VseTitleLabel.Unbind();

            if (graphModel?.AssetModel is GraphAssetModel model)
            {
                m_VseTitleLabel.bindingPath = "m_Name";
                m_VseTitleLabel.Bind(new SerializedObject(model));
            }
        }

        public void DisplayAppropriateSearcher(Vector2 mousePosition)
        {
            m_LastProvider.DisplayAppropriateSearcher(mousePosition, GraphView);
        }
