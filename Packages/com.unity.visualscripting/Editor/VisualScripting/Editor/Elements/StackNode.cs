...
{
    public class StackNode : UnityEditor.Experimental.GraphView.StackNode, IHasGraphElementModel, ICustomColor, IMovable, IVSGraphViewObserver
    {
        ...
        public StackNode(Store store, IStackModel stackModel, INodeBuilder builder)
        {
            ...
            // TODO: Should be useless/obsolete with the new renderer
            borderItem.cacheAsBitmap = false;
        ...
        void UpdateFromModel()
        {
            ...
            UpdatePortsFromModels(stackModel.InputPortModels, "input");
            UpdatePortsFromModels(stackModel.OutputPortModels, "output");
        ...
        void UpdatePortsFromModels(IReadOnlyList<IPortModel> portModelsToCreate, string portCollectionName)
        {
        ...
        protected override void OnSeparatorContextualMenuEvent(ContextualMenuPopulateEvent evt, int separatorIndex)
        {
            ...
            evt.menu.AppendAction("Split Stack", menuAction =>
                ...
                }, action => separatorIndex == 0 ? DropdownMenuAction.Status.Disabled : DropdownMenuAction.Status.Normal);
        }
