...
namespace UnityEditor.VisualScriptingTests.UI
{
    class CreationTests : BaseUIFixture
    {
        ...
        [UnityTest]
        public IEnumerator Test_CreateNodeAtTheRequestedPosition()
        {
            Vector2 position = new Vector2(23, 45);

            ((TestState)Store.GetState()).queryGraphNodeModelMockResult = data =>
            {
                var n = ((VSGraphModel)data.graphModel).CreateNode<Type0FakeNodeModel>("n", data.position);
                return new IGraphElementModel[] { n };
            };

            Store.Dispatch(new CreateNodeFromSearcherAction(GraphModel, Vector2.zero, position));
            yield return null;

            Assert.That(GetGraphElements().Count, Is.EqualTo(1));
            Assert.That(GetGraphElement(0).GetPosition().position, Is.EqualTo(position));
            yield return null;
        }
        ...
        public IEnumerator Test_CreateStackWithNodes()
        {
            ...
            stackModel.CreateStackedNode<Type0FakeNodeModel>("Node0", 0);
            stackModel.CreateStackedNode<Type1FakeNodeModel>("Node1", 0);
