...
#pragma warning restore CS0414
        ...
        void CreateNodesAndValidateGraphModel(GraphNodeModelSearcherItem item, NodeCreationMode mode,
            Action<List<INodeModel>> assertNodesCreation)
        {
            ...
            if (mode == NodeCreationMode.Orphan)
            {
        ...
        void CreateNodesAndValidateStackModel(StackNodeModelSearcherItem item, NodeCreationMode mode,
            Action<List<INodeModel>, IStackModel> assertNodesCreation)
        {
            ...
            item.CreateElements.Invoke(new StackNodeCreationData(stack, -1, mode));
        ...
        [TestCase(NodeCreationMode.GraphRoot)]
        [TestCase(NodeCreationMode.Orphan)]
        public void TestGraphVariables(NodeCreationMode mode)
        {
        ...
        [TestCase("v", typeof(int), NodeCreationMode.GraphRoot)]
        [TestCase("p", typeof(string), NodeCreationMode.GraphRoot)]
        [TestCase("p", typeof(string), NodeCreationMode.Orphan)]
        public void TestFunctionMembers(string query, Type type, NodeCreationMode mode)
        {
            ...
            funcModel.CreateFunctionParameterDeclaration("par", typeof(string).GenerateTypeHandle(GraphModel.Stencil));
        ...
        [TestCase(SearcherContext.Stack, "set variable", typeof(SetVariableNodeModel), NodeCreationMode.GraphRoot)]
        [TestCase(SearcherContext.Stack, "set variable", typeof(SetVariableNodeModel), NodeCreationMode.Orphan)]
        [TestCase(SearcherContext.Graph, "make", typeof(MakeArrayNodeModel), NodeCreationMode.GraphRoot)]
        [TestCase(SearcherContext.Graph, "make", typeof(MakeArrayNodeModel), NodeCreationMode.Orphan)]
        public void TestNodesWithSearcherItemAttributes(SearcherContext context, string query, Type type, NodeCreationMode mode)
        {
            var db = new GraphElementSearcherDatabase(Stencil)
        ...
        [TestCase(SearcherContext.Graph, "g", SearcherItemTarget.Group)]
        ...
        [TestCase(NodeCreationMode.Orphan)]
        [TestCase(NodeCreationMode.GraphRoot)]
        public void TestBinaryOperator(NodeCreationMode mode)
        {
            var db = new GraphElementSearcherDatabase(Stencil)
                .AddBinaryOperators()
                .Build();
        ...
        [TestCase(SearcherContext.Graph, "mi", UnaryOperatorKind.Minus, NodeCreationMode.GraphRoot)]
        [TestCase(SearcherContext.Graph, "mi", UnaryOperatorKind.Minus, NodeCreationMode.Orphan)]
        [TestCase(SearcherContext.Stack, "postD", UnaryOperatorKind.PostDecrement, NodeCreationMode.GraphRoot)]
        [TestCase(SearcherContext.Stack, "postD", UnaryOperatorKind.PostDecrement, NodeCreationMode.Orphan)]
        [TestCase(SearcherContext.Stack, "postI", UnaryOperatorKind.PostIncrement, NodeCreationMode.GraphRoot)]
        [TestCase(SearcherContext.Stack, "postI", UnaryOperatorKind.PostIncrement, NodeCreationMode.Orphan)]
        [TestCase(SearcherContext.Graph, "lo", UnaryOperatorKind.LogicalNot, NodeCreationMode.GraphRoot)]
        [TestCase(SearcherContext.Graph, "lo", UnaryOperatorKind.LogicalNot, NodeCreationMode.Orphan)]
        public void TestUnaryOperators(SearcherContext context, string query, UnaryOperatorKind kind,
            NodeCreationMode mode)
        {
            var db = new GraphElementSearcherDatabase(Stencil)
                .AddUnaryOperators()
                .Build();
        ...
        [TestCase(0, NodeCreationMode.GraphRoot)]
        [TestCase(0, NodeCreationMode.Orphan)]
        [TestCase(1, NodeCreationMode.GraphRoot)]
        [TestCase(1, NodeCreationMode.Orphan)]
        [TestCase(2, NodeCreationMode.GraphRoot)]
        [TestCase(2, NodeCreationMode.Orphan)]
        public void TestIfCondition(int index, NodeCreationMode mode)
        {
            var db = new GraphElementSearcherDatabase(Stencil)
                .AddControlFlows()
                .Build();
            ...
            CreateNodesAndValidateStackModel(item, mode, (initialGraphNodes, stack) =>
            {
                ...
                switch (index)
                {
                    ...
                    case 1:
                    {
                        ...
                        var thenStack = GraphModel.NodeModels.OfType<StackModel>().FirstOrDefault(n => n.name == "then");
                        Assert.IsNotNull(thenStack);
                        Assert.That(node.OutputPortModels[0], Is.ConnectedTo(thenStack.InputPortModels[0]));

                        var elseStack = GraphModel.NodeModels.OfType<StackModel>().FirstOrDefault(n => n.name == "else");
                        Assert.IsNotNull(elseStack);
                        Assert.That(node.OutputPortModels[1], Is.ConnectedTo(elseStack.InputPortModels[0]));
                    ...
                    case 2:
                    {
                        ...
                        Assert.That(lastStack.InputPortModels[0], Is.ConnectedTo(elseStack.OutputPortModels[0]));
                        Assert.That(lastStack.InputPortModels[0], Is.ConnectedTo(thenStack.OutputPortModels[0]));
